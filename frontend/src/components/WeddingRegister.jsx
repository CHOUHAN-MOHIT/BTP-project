...
const WeddingRegister = ({ closeModal }) => {
    ...
    if (formStep < 3) {
      return (
        <div className="flex justify-between mt-4">
          {formStep > 0 ? (
            <button
              type="button"
              onClick={() => handleFormNav(-1)}
              className="bg-orange-600  px-4 py-3 rounded text-md w-1/2 mr-3"
            >
              Prev
            </button>
          ) : (
          ...
          <button
            type="button"
            onClick={() => handleFormNav(1)}
            className="bg-orange-600  px-4 py-3 rounded text-md w-1/2"
          >
            Next
          </button>
        </div>
      ...
      return (
        <div className="btn-grp flex justify-between mt-4">
          ...
          <button
            type="submit"
            onClick={handleWeddingRegistration}
            className=" bg-orange-600  px-4 py-3 rounded text-md w-1/2"
          >
            Register
          </button>
  ...
  const addEventSection = () => {
    setFormData((prevData) => ({
      ...
      events: [
        ...prevData.events,
        { name: "", date: null, time: null, location: "", description: "" },
      ],
    }));
  };
    ...
    <div className="min-h-screen p-6 flex flex-col items-center justify-center">
      <div className="container max-w-screen-lg mx-auto bg-white rounded">
        <div className="flex flex-row-reverse p-2">
          <button
            onClick={closeModal}
            className="z-10 opacity-50 hover:opacity-100 right-0"
          >
              ...
              <div className="lg:col-span-2 overflow-y-hidden max-h-screen">
                ...
                {formStep === 2 && (
                  <section className="events-section overflow-y-auto max-h-[80vh]">
                    {formData.events.map((event, index) => (
                      <div
                        key={index}
                        className="event-card rounded-lg shadow-md bg-white p-4 mb-4"
                      >
                    ...
                    <button
                      type="button"
                      onClick={addEventSection}
                      className="px-3 py-2 bg-orange-500 text-white rounded hover:bg-orange-600"
                    >
                      <span className="font-semibold text-lg"> + </span> Add
                      Event
                    </button>
                  </section>
                )}
                {formStep === 3 && (
                  <>
                    <section className="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                      <div className="md:col-span-5">
                        <label for="invitationCard" className="block">
                          Couple's Picture
                          <input
                            type="file"
                            name="invitationCard"
                            ...
                            onChange={(e) =>
                              setFormData({
                                ...formData,
                                invitationCard: e.target.files[0],
                              })
                            }
                          />
                        </label>
